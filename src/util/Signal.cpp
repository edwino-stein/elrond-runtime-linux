...
std::map<enum SIG, signalCallbackT> Signal::callbacks;

Signal::Signal(){}

void Signal::attach(enum SIG sig, signalCallbackT task){
    Signal::detach(sig);
...
bool Signal::hasAttached(enum SIG sig){
    std::map<enum SIG, signalCallbackT>::iterator it = Signal::callbacks.find(sig);
